<template>
    <div class="adoption-feed-container">
        ...
        <!-- Navbar (Asumo que está importado y debe ir aquí, antes del div principal) -->
        <Navbar /> 
        ...
        <div class="content-wrapper feed-content">
            ...
            <div v-else-if="applyFilters.length > 0" class="pet-card-grid">
                ...
                <PetCard v-for="mascota in applyFilters" :key="mascota.idxxxx_mascot" :mascota="mascota" />
            </div>
...
// ==============================================
// 1. OBTENCIÓN DE DATOS DEL BACKEND (REAL)
// ==============================================

/**
 * Función para obtener TODAS las mascotas del feed desde la nueva ruta optimizada.
 */
async function getMascotas() {
    ...
    try {
        ...
        // Asigna los datos reales del backend
        mascotas.value = data;
        
    } catch (error) {
        console.error("Error al obtener las mascotas:", error);
        // Dejar el array vacío para que se muestre el mensaje de "No Results"
        mascotas.value = []; 
    } finally {
...
const applyFilters = computed(() => {
    ...
    if (filters.busqueda) {
        ...
        // Usar los nombres de columna de la base de datos: 'nombre_mascot' y 'razaxx_mascot'
    ...
    switch (filters.orden) {
        case 'alfabetico':
            ...
            break;
        case 'mas_antiguos':
            // Usar ixxxx_mascot para ordenar
...
onMounted(() => {
    // Aseguramos que el token esté cargado antes de hacer la llamada a la API
    if (!authStore.token) {
...
const PetCard = ({ mascota }) => {
    // Usamos mascota.edadme_mascot para el formato
    const ageDisplay = formatAge(mascota.edadme_mascot);
    // Usamos mascota.image1_mascot para la imagen
    ...
    return h('div', { class: 'pet-card' }, [
        ...
        h('div', { class: 'pet-card-content' }, [
            ...
            h('div', { class: 'pet-card-chips' }, [
                ...
                h('span', { class: 'pet-card-breed' }, mascota.razaxx_mascot || 'Mestizo'), // Usamos razaxx_mascot
            ]),
...
/* Tarjeta Individual */
.pet-card {
    background-color: #e95e5e; /* bg-white */
...
.pet-card-image-container {
    height: 5rem; /* h-48: Define la altura del contenedor */
    width: 100%;
    overflow: hidden;
